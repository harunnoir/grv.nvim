...
local function get_background_palette(background)
...
local function get_foreground_palette(foreground)
  if foreground == 'material' then
    if is_dark() then
      return {
        fg0       = { '#d4be98', '223' },
        fg1       = { '#ddc7a1', '223' },
...
local function get_grey_palette()
  if is_dark() then
    return {
      grey0 = { '#7c6f64', '243' },
      grey1 = { '#928374', '245' },
...
function M.get_palette(config)
  local bg_palette = get_background_palette(config.background)
  local fg_palette = get_foreground_palette(config.foreground)
  local grey_palette = get_grey_palette()

  local palette = vim.tbl_extend('force', bg_palette, fg_palette, grey_palette)

  if config.colors_override and next(config.colors_override) ~= nil then
    palette = vim.tbl_extend('force', palette, config.colors_override)
  end
